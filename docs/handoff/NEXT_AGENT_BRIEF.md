...
先读 docs/handoff/WORKSPACE_STATE_2026-09-14.md。正式开发基线是 origin/main 的 06e73c3；主目录已整理到该版本。产品入口使用 1.1.5，旧版 ZIP 和废弃分享 QA 已从当前工作树移除；正式站旧版直链仍返回 200，待下一次发布清理。PRD、设计记录、交付物和宣传片源文件保留但不混入产品发布提交。

## 当前产品快照（优先于下方历史说明）

- 线上主路径是 `/`、`/explore`、`/my`、`/bottle`、`/resume`、`/forum`、`/extension`、`/login`、`/profile`、`/feedback`、`/referrals`；下方出现的 `/jobs`、`/my-applications`、`/my-bottle` 是历史路由记录。
- 页面工作区采用浅银蓝主题；主页保留线上星系导航与轨道动效，工作页使用柔和边界、统一控件反馈和紧凑岗位列表。
- 网申助手当前版本为 1.1.5；旧安装包已从 public/downloads 移除，安装包一致性只校验当前版本。
- 星瓶分享海报功能已撤下；相关旧分享源码和 QA 记录只在历史提交中保留。
