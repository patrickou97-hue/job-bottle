...
- 简历制作中的教育经历、实习与工作经历、项目经历支持按条目自定义顺序：左侧拖拽把手可直接排序，同时提供上移、下移和删除按钮作为键盘/触控兜底。
- 排序沿用现有 `touchResume`、本地/云端自动保存、实时预览和 PDF 导出链路，数组顺序就是最终简历呈现顺序；提交 `4725214` 已推送 `origin/main`，线上 `/resume` 脚本已核验新交互标记。

## 2026-09-19 管理后台 v3 全局重构与权限首屏修复（已发布）

- 全局复核并重构七个后台路由：数据分析、反馈管理、岗位管理、用户管理、内推码管理、计费与批量导入。统一为紧凑运营工作区、冷白纸面、深蓝主色、细边界和明确的操作层级，收紧卡片圆角与间距，移除大面积空白和模板化 AI 卡片感。
- 所有主要模块补齐结构化骨架加载态、表格/列表/筛选器/按钮的统一几何和错误边界；反馈与内推码在 1024px 以上使用四列信息布局，平板与手机安全收敛为两列或单列，避免截断和错位。
- `AdminShell` 改为服务端先执行 `requireAdminAccess`，把初始权限结果直接传给客户端，修复 Safari 路由切换后先出现三个孤立圆点或长期权限等待的问题；`/api/admin/session` 保留为客户端刷新边界。
- 第二轮精修将数据分析四张独立指标卡改为连续指标仪表条，以统一边界和细分隔线提高信息密度；平板与手机保持两列安全降级。
- 首页按最终审美确认移除“核心模块 / 快捷动作 / 低频工具 / 权限隔离”四段概览条，工作模块区直接承接首屏内容；提交 `222bea2` 已推送并完成线上 HTML 复核。
- 发布提交：`142cff3`、`f28b993`、`ee653a9`、`41cecde`、`91120c0`，均已推送 `origin/main`。本轮没有新增业务数据写入。
- 最新验证：typecheck、lint、Webpack 生产构建和 diff check 通过；测试套件 183/183 通过。Safari 登录态的反馈页在缓存破除 URL 下已复核为完整列表，匿名管理 API 仍返回 401。

## 2026-09-19 管理后台全局视觉审计与加载态修复（已上线）

- 已登录 Safari 复核全部七个管理路由：数据分析、反馈管理、岗位管理、用户管理、内推码管理、诘星计费和批量导入均能加载；品牌字标素材路径已核验，无错误素材引用。
- 新增 `/api/admin/session` 服务端会话核验，修复后台路由切换后的权限加载卡住；反馈骨架改为与真实列表对齐的行结构，修复“三个点”错排；反馈和内推码列表在中等宽度下折叠为两列，宽屏才展示四列，避免内容溢出；所有后台路由补齐页面标题与 noindex。
- `433522c`、`b589ace` 已推送 `origin/main`；Vercel deployment `4stsSkCBDQNaSeWWnmtdRWEJRoQy` 成功。后台页面 200，匿名管理 API 401，登录态会话接口返回管理员身份 JSON。
- typecheck、lint、183/183 测试、Webpack 生产构建（64 个静态页面生成）与 diff check 通过；未新增业务数据写入。
...
正式基线为 origin/main 的 06e73c3；主工作区已对齐该版本。旧版扩展 ZIP 与废弃分享 QA 已从当前工作区移除，产品入口指向 1.1.5；正式站旧版直链仍返回 200，待下一次发布清理。未提交的 PRD、设计记录、交付物和宣传片源文件属于受保护资料，详见 docs/handoff/WORKSPACE_STATE_2026-09-14.md。

当前运行路由与视觉快照优先于本文旧版记录：用户端使用 `/explore`、`/my`、`/bottle`、`/resume`、`/forum`、`/extension`、`/profile`、`/feedback`、`/referrals`；旧安装包、旧路由和星瓶分享海报只保留为历史证据。

## 2026-09-18 管理后台与用户导航发布（已上线）

- 发布范围：管理员后台共享壳层、岗位管理抽屉/分页/局部更新、后台顶栏“更多”菜单，以及用户端工作台顶栏“更多”中的“内推码广场”入口。
- Git 与部署：管理员改动提交 `c423010`，用户导航补丁提交 `7fc53ed`，均已推送 `origin/main`；Vercel 已完成最新版本部署，线上内容以当前 `origin/main` 为准。
- 线上探针：`https://www.starjob.space/explore` 返回 HTTP 200，HTML 已检出“更多”和“内推码广场”；`/admin` 返回 HTTP 200；匿名 `GET /api/admin/feedback` 与 `GET /api/admin/users` 均返回 401，管理员鉴权边界保持有效。
- 未确认项：没有管理员真实登录态的浏览器 E2E、岗位编辑写入和生产设备视觉验收；本次没有新增数据库、migration、RLS 或用户数据写入。


## 2026-09-18 用户端顶栏“更多”补充内推码广场（待发布）

- 用户目标：将用户界面的内推码广场入口收进顶部“更多”菜单，不占用主导航位置。
- 实际改动：`src/components/layout/Navbar.tsx` 将“内推码广场”加入桌面与移动端共享的更多菜单，并放在菜单首项；访问 `/referrals` 时沿用当前菜单高亮与路由关闭行为。
- 兼容边界：未改变内推码数据、上传、举报、权限、API 或移动底部导航；本次只调整入口位置。
- 本地验证：`npm run typecheck`、用户导航定向 ESLint 与 `git diff --check` 通过；本次入口补丁尚未重新执行生产构建、部署和线上探针。


## 2026-09-17 管理后台第一批 UI 优化（本地已验证，未发布）

- 用户目标：执行管理员后台优化方案，先改善高频的导航、岗位列表与编辑流程，并将内推码收纳到顶栏的“更多”。
- 实际改动：`/src/app/admin/layout.tsx` 统一承载 `AdminShell`，各管理员页面不再重复挂载壳层；`AdminShell` 将内推码、批量导入和计费收进顶栏“更多”菜单，移动端保留同一组入口；`AdminJobsClient` 将新增/编辑表单改为侧边抽屉，支持 Esc 与遮罩关闭，岗位列表增加 40 条分页、结果计数和局部上下架/删除更新，避免每次操作整表刷新；后台样式增加柔和边界、表格悬停、分页和抽屉动效，并尊重 `prefers-reduced-motion`。
- 兼容边界：保留现有管理员权限核验、岗位重复检测、搜索、批量导入入口、投递链接清洗、删除确认和业务接口；未新增数据库、migration、RLS、API、用户数据或部署。
- 本地验证：修改文件定向 ESLint 通过，`npm run typecheck` 通过，`git diff --check` 通过。尚未进行真实管理员登录态浏览器验收、生产部署或线上探针。


## 2026-09-14 全局 UI 与动效性能发布候选

- 基于线上 6fcf970 合入本轮界面修改：浅色柔和边界、统一控件反馈、登录页中英文字标及行星前后遮挡、导航更多菜单、指南交互与投递按钮对齐。主页保持线上版本，扩展继续使用已上线 1.1.5。
- 岗位仅在存在有效内推码时显示入口，快捷查看支持有内推码企业。内推码数据库与公开来源并行读取，本地超时回退仍保留公开来源。
- 登录行星改用 transform 移动与 ResizeObserver 缓存尺寸；不可见时停止 RAF，支持暂停和减少动态效果。
- 发布候选验证：181 项测试通过，typecheck、Webpack 生产构建通过；lint 0 error、1 条既有 warning。浏览器验证运动、暂停、继续、减少动态效果和 390px 无横向溢出；1440px/390px 内推筛选均返回匹配记录。未执行真实账号登录态业务 E2E。生产部署结果以本次发布后探针为准。

## 2026-09-14 公开内推码多渠道采集与广场入库（已落库，未部署）

- 在读取当前项目记录、实现和生产岗位库后，沿用既有官方来源通道：截至 2026-09-17 当前有效岗位 2,093 条、企业名/标识 1,977 个；本轮追加前官方来源为 168 条/76 个企业名，本轮新增 6 条、覆盖 6 个企业名，落库后为 174 条/82 个企业名。新增来源按企业名+代码去重，发布者固定为“拾星小助手整理”，保留平台、公开原帖、届次说明和核对提示，不冒充用户投稿。
- 本轮新增均来自牛客公开帖：三七互娱 DStcT227、欣旺达 EZ1P20、三环集团 878388、京东方 FY34V、深蓝互动 DSGGU24P、阳光电源 NTAuBxF。各条均明确写出 27 届/2027 校招和可复制内推码，并保留原帖链接；使用前仍需在企业官方投递页核对有效期，不把代码有效性或录用结果作保证。大华、海康、京东、小米、携程等本轮检索到但已有来源的企业跳过重复入库。
- 线上验证：服务角色只读回本轮 6 条，全部 active，企业名+代码唯一；当前官方来源表为 174 条、覆盖 82 个企业名。`/api/referrals/source` 进程缓存为 3 小时，直接写表不会强制刷新同一实例缓存；本轮未执行部署或强制刷新。
- 智联招聘本轮仍未找到同时满足 27 届/2027 秋招、明文可复制代码和公开可追溯链接的合格条目，因此没有补写猜测值；二维码-only、私聊/付费、旧届、仅实习和无法与当前岗位库精确匹配的企业别名同样排除。当前岗位库仍有 1,895 个企业名/标识尚无官方来源码，本批不宣称“每个企业都已覆盖”。本次没有社交平台发帖、评论、私信，也没有修改业务代码、提交、推送或部署。

## 2026-09-12 网申助手 1.1.5 B 站控件与月份边界稳定性修复（已上线）

- 真实哔哩哔哩网申页暴露了三个稳定性缺口：自定义性别/学历下拉可能因异步弹层而漏填；动态控件失败会拖累后续自我描述；简历仅含年月时，完整结束日期会错误使用月初。1.1.5 将普通文本提前写入、动态控件逐字段重试并保留安全自我描述回退；AI 字段匹配优先使用 MiMo，只有未配置 MiMo 时才使用 DeepSeek 兼容路径。
- 日期策略明确为：来源只有年月且网页要求完整日期时，开始月使用 1 日、结束月使用当月最后一天；网页只接收年月时仍保留 YYYY-MM。AI 写入分支现在传递完整字段语义，原生 date/month 控件不再被周边布局误判为拆分的年/月/日控件；只读完整日期控件可在选日器无法可靠定位时写入并回读验证。
- 安全边界不变：只处理普通且可核对字段，不填写身份证、密码、验证码、声明、附件或提交按钮；AI 开放文本必须有简历证据，无法确认的字段保持手动。真实哔哩哔哩页面仅做未提交填写验证，没有保存、上传或提交。
- 发布前验证：`npm test` 181/181、扩展契约 15/15、Chrome 真实写回夹具 21/21、B 站专项夹具 1/1、typecheck、lint（0 error、1 条既有 warning）、Webpack production build（62 个静态页面）、只读 smoke、12/12 安装包一致性和 diff check 均通过。Smoke 固定使用 webpack，避免隔离 worktree 的外部 node_modules 符号链接触发 Turbopack panic。
- 发布提交 `be7a2f6` 已推送 `origin/main`。正式 `/extension`、`/extension/guide` 与 1.1.5 ZIP 均返回 HTTP 200；线上 ZIP 为 255561 bytes，SHA-256 `5096b94a0891e1660e4f0e6eb76c8534fc96b82ca3a740d7a9fd064273f58564`，与本地包一致。匿名 autofill POST 返回预期 401。

## 2026-09-10 网申助手 1.1.4 多段经历描述错位修复（已上线）

- 用户截图确认：公司、项目名称等硬事实已能落到正确记录，但第二段实习会写入第一段实习描述，项目记录也会写入另一项目描述。根因是旧服务端只验证生成内容在整份简历中有依据，没有验证依据属于当前记录；扩展同时会把整份简历发送给单个叙述字段批次，增加了跨记录匹配概率。
- 1.1.4 对实习、工作、项目、校园和获奖叙述按记录隔离：每个叙述批次只向 AI 暴露目标记录；返回证据必须全部落在对应 `resumePath` 的同一记录，并与该记录内容具有非通用文本锚点。校验失败时只回退到当前记录的精确描述，不再接受整份简历中的相似内容。
- 验证：`npm test` 180/180；扩展浏览器夹具 21/21，覆盖三种填写模式、多段实习、两段项目、编号式 ATS、重渲染和哔哩哔哩式自定义控件；typecheck、lint（0 error、1 条既有 warning）、Webpack build（62 routes）、smoke、12/12 安装包一致性和 diff check 均通过。针对跨记录叙述新增正反单测与执行顺序契约测试。
- 发布提交 `d7b3c50` 已推送 `origin/main`。正式 `/extension`、`/extension/guide` 与 1.1.4 ZIP 均返回 HTTP 200；线上 ZIP 为 251848 bytes，SHA-256 `23fcdd093eaa0454aa2b89e6bb24b56c7b8222a86fb5d2e823d2d37cf7dc01c0`，与本地包一致。匿名 autofill POST 返回预期 401。没有保存或提交第三方网申表单；更新扩展后在真实登录态页面清空并重填错位字段的设备验收仍未执行。

## 2026-09-10 网申助手 1.1.3 学校/公司字段错位修复（已上线）

- 真实哔哩哔哩 ATS 复测确认，学校和公司输入框的可见标签与控件分离，控件只保留占位符；旧属性匹配器又排除了 4 个汉字的别名。更关键的是，日期配对逻辑会对记录卡片中的所有普通文本框运行，把学校、公司和职位误判为起止日期。
- `fill.js` 现在允许至少 2 个非 ASCII 字符的明确属性别名，并且只有日期类型、日期语义属性、年/月原生选择器或只读日期触发器才进入起止日期配对。基础硬事实的本地、执行端和服务端接受阈值统一为 0.74。
- 新增受控文本框和带可见记录序号的 ATS 回归夹具。`npm test` 178/178，typecheck、build（62 routes）、smoke、扩展构建、12/12 包一致性和 `git diff --check` 均通过；21 个浏览器夹具已分组或逐项通过。真实 Chrome 登录态 AI 流程完成 16/16 批次，页面实读学校、4 段公司名称及对应职位均已写入，0 项写入失败；未点击保存、取消、上传或提交。
- 发布提交 `e75855beebfbb5172d516f7ead7759341e82e899` 已推送 `origin/main`。正式 `/extension` 与 1.1.3 ZIP 返回 HTTP 200；线上 ZIP 为 250231 bytes，SHA-256 `5d29da3bbfe2bc820564b7f4fd2cef623c889f0d11acb8e622cd655e02c25c0a`，与本地最终包逐字节一致。匿名 autofill POST 返回预期 401，不把该探针冒充登录态 E2E。

## 2026-09-08 网申助手 1.1.2 写回稳定性与字段防错位（已上线）

- 用户反馈与真实页面证据：哔哩哔哩校招简历页提取 62 个可见字段，服务端返回并通过 38 个映射，但最终仅写入 8 项、34 项写入失败；页面同时出现手机号重复 `86` 区号、学校名称被写成 `2027.08`、项目名称被写成校园经历、作品链接被写成课程列表，以及多段经历只写入首个公司名。AI 并非没有返回，主要故障发生在结果编译和浏览器写回阶段；本轮未取得可关联该请求的 Vercel 服务端日志，因此不把浏览器证据冒充日志证据。
- 根因与决策：服务端 `parseResult` 和扩展 `fill.js` 会分别用本地 `deterministicKey` 覆盖一次合格 AI 结果；服务端最低置信度已为 0.68，但 popup 和执行层仍要求 0.82；AI 分析期间页面或 iframe 重渲染后仍使用旧 frame/field 地址；自定义下拉等待仅 1.2 秒，回读失败也没有进入 `failed` 统计。
- 实际改动：三层统一使用 0.68 接受阈值，合格 AI 结果作为执行计划，本地结构化值只在模型未回答时兜底；基础信息与每条教育、工作、项目记录改用字段级事实范围，新增学校/专业/公司/职位、日期、手机、邮箱和 URL 类型兼容校验。写入前重新稳定扫描页面，以 element identity、原始键、区块、记录序号、语义键和标签唯一重绑定到最新 frame/控件；无法唯一重绑定的字段单独转人工。自定义下拉等待增至 2.8 秒并补充父触发器；中国手机号若模型返回 `+86`/ `86` 加 11 位号码，写入号码框前去除重复国家码；回读不一致现在如实计入失败。
- 版本与文件：扩展 manifest 升为 1.1.2；官网下载安装入口和教程指向 `public/downloads/starjob-resume-assistant-v1.1.2.zip`。新增 `src/lib/extension-autofill-field-compat.ts`、字段兼容单测和哔哩哔哩式自定义控件夹具；安装包 222506 bytes，SHA-256 `3f67ee66b546500b5614b2752c312355a12688de4f69add1de41c9de6d58c082`，包内 12/12 文件与源码逐字节一致。
- 验证：`npm test` 178/178；沙箱外真实 Chrome headless `npm run test:extension` 16/16，覆盖只填空白、覆盖已有、AI 智能填写、四段实习、两段项目、重复日期、填写中重渲染、延迟 hydration、自定义下拉和手机号区号；`npm run lint` 0 errors、1 条既有 warning；`npm run typecheck`、`npm run build -- --webpack`（62 routes）、`npm run smoke`、`npm run build:extension`、`npm run verify:extension-package`、两份扩展脚本 `node --check` 与 `git diff --check` 全部通过。
- Git、部署与生产状态：功能提交 `2104b0b31a7a2098bd6772981fdbd2c82292044d` 已从隔离发布树推送到 `origin/main`；Vercel 部署 [AyGbxLgjQ7B4H3zq7imKsTHYvE9c](https://vercel.com/job-bottle/job-bottle/AyGbxLgjQ7B4H3zq7imKsTHYvE9c) 返回 `success / Deployment has completed`。正式站 `/extension`、`/extension/guide` 和 `/downloads/starjob-resume-assistant-v1.1.2.zip` 均返回 HTTP 200，页面检出 1.1.2 下载地址；线上 ZIP 为 222506 bytes，SHA-256 `3f67ee66b546500b5614b2752c312355a12688de4f69add1de41c9de6d58c082`，与发布包逐字节一致。匿名 `POST /api/resume/extension-autofill` 返回 401。
- 边界与状态：没有新增数据库、migration、RLS、权限或自动提交能力，没有提交第三方网申表单。敏感字段仍不填写；AI 改写优先不等于绕过字段类型、记录范围、事实和敏感字段校验。真实登录态 AI 请求、哔哩哔哩生产页面重新安装 1.1.2 后的最终设备验收仍未执行。


## 2026-09-08 网申助手 AI 返回容错热修复（已上线）

- 用户反馈与根因：生产环境偶发提示“AI 返回内容不完整，系统已尝试补齐”。原因包括模型输出被截断、少数字段缺失/重复/格式轻微偏差，以及旧修复队列按调用次数而非修复轮次计数，拆分后的修复分组会提前耗尽补齐机会。
- 实际改动：单批输出预算提高到 5000 tokens；支持从截断 JSON 中安全提取已闭合结果，兼容常见 fieldKey、状态、动作、置信度、值类型、依据与证据路径格式差异；重复字段保留第一条通过结构校验的结果。修复队列改为按轮次补齐，同一轮分组并行，修复上游异常仅让对应字段进入下一轮或人工确认。
- 容错与安全边界：最低模型置信度从 0.82 调整为 0.68；有明确依据类型但缺省置信度时使用 0.72。少数字段最终仍不完整时，不再整批失败，已通过事实与结构校验的字段继续填写，缺失字段标为人工确认。验证码、密码、证件、隐私/法律声明、薪资等敏感字段，以及跨经历记录、错误日期、无证据数字和虚构事实仍然硬拦截；不自动提交。
- 验证：`npm run typecheck`、`npm test`（173/173）、`npm run lint`（0 errors，1 条既有 warning）、`npm run test:extension`（15/15）、`npm run build -- --webpack`（62 routes）、`npm run smoke` 和 `git diff --check` 通过。未进行携带真实简历的登录态 AI E2E 或第三方 ATS 实际写入。
- Git、部署与生产状态：功能提交 `460a4c1` 已推送到 `main`，触发 Vercel 生产部署；现有 1.1.1 扩展无需重新安装。生产匿名 `POST /api/resume/extension-autofill` 返回 401；该探针只验证鉴权边界，不等同于登录态 AI E2E。

## 2026-09-08 网申助手连续测试限流热修复（已上线）

- 线上症状与根因：Chrome 真实扩展在 KPMG Moka 页面已完成第 1 批后，第 2 批返回“AI 智能填写请求较频繁”；当时页面共 8 批、74 个安全字段，未开始写入。原因不是模型内容不完整，而是 1.1.1 同时把单次操作上限和十分钟滚动批次窗口都设为 100，连续测试后耗尽用户额度。
- 实际改动：新增迁移 `20260908180000_raise_extension_autofill_rolling_batch_window.sql`，单次操作仍最多 100 批，十分钟滚动窗口提升为 500 批，并继续限制最多 5 个活跃操作；敏感字段过滤、串行批次、全批成功后统一写入和零写入失败保护不变。不需要重新安装扩展。
- 验证：迁移已在托管 Supabase 执行，远端迁移列表显示 `20260908180000` 已执行；迁移契约测试、`npm test`（171/171）、`npm run smoke` 和 `git diff --check` 通过。未进行携带真实简历的登录态 AI E2E 或第三方 ATS 实际写入。
- Git、部署与生产状态：热修复提交 `a84897f` 已推送到 `main`；扩展包仍为已上线的 1.1.1，线上匿名 `POST /api/resume/extension-autofill` 仍返回 401。
...
- Git、部署与外部状态：功能提交 `307aa035bca83e1c816baf8adfa357e223d30552` 已从最新 `origin/main` 的隔离克隆推送到 `main`；Vercel 部署 `AhCX5zZvFLzddRaoahTLY8JZr47L` 返回 `success / Deployment has completed`。正式站 `/extension`、`/extension/guide` 与 `/downloads/starjob-resume-assistant-v1.0.1.zip` 均返回 HTTP 200，页面检出 1.0.1 下载地址与“当前版本 1.0.1”；线上 ZIP 为 210083 bytes，SHA-256 `de59eacfe67c0257a065a6b74cd0d2273a84dea18ef807c5c3dd7f5c1add1c07`，与发布包一致。匿名 `POST /api/resume/extension-autofill` 返回 401，未把该探针当作登录态 AI E2E。
...
- Git 与部署证据：发布提交 `7538a76a042e75f57339c31b20bfb4cacb9a7e48` 与最终修正提交 `0972772` 已推送到 `origin/main`；最终 Vercel 部署 [9grXyoKWQ8yYgrNN9Bq2aC6z7zRS](https://vercel.com/job-bottle/job-bottle/9grXyoKWQ8yYgrNN9Bq2aC6z7zRS) 返回 `success / Deployment has completed`；本地生产预览已重启在 `http://localhost:3120/`。
