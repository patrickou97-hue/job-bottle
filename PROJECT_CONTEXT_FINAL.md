...
- 验证与发布：`npm run typecheck`、`npm run lint`、`npm test`（136/136）、`npm run build`（58 个页面）、`npm run smoke` 和 `git diff --check` 已通过；本地回归覆盖排序默认值、重置值、筛选状态、比较方向和选择器提示。代码尚未提交、推送或部署，正式网页探针待上线后补录；本地预览与线上数据均不涉及写入。
