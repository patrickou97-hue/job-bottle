...
test("MiMo base configuration uses the callable v2.6 Flash id", () => {
  assert.equal(resolveMimoModel("mimo-v2.5"), CURRENT_MIMO_MODEL);
  assert.equal(resolveMimoModel(" MIMO-V2.5 "), CURRENT_MIMO_MODEL);
  assert.equal(resolveMimoModel("mimo-v2.6"), CURRENT_MIMO_MODEL);
});

test("current and non-Pro model identifiers are preserved", () => {
  assert.equal(resolveMimoModel("mimo-v2.6-pro"), "mimo-v2.6-pro");
  assert.equal(resolveMimoModel("mimo-v2.6-flash"), "mimo-v2.6-flash");
