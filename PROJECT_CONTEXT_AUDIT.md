...
## 2026-09-06 浏览器 favicon 改为拾星黑色文字标（本地待发布）
...
- 验证：资源格式与尺寸检查通过；生成图视觉检查通过；待在该隔离分支接入现有环境后执行 `npm run typecheck`、`npm test`、`npm run lint`、`npm run build -- --webpack` 与正式站缓存穿透探针。
- 当前状态：改动位于隔离工作树 `/private/tmp/starjob-favicon` 的本地分支 `codex/favicon-wordmark`，尚未合并、推送或部署。
