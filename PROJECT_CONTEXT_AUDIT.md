...
- 审计范围：简历编辑器的教育经历、实习/工作经历和项目经历条目；每组均有拖拽把手、上下按钮和删除操作，排序回写现有简历内容数组。
- 发布证据：`4725214` 已推送 `origin/main`；线上 `/resume` 返回 200，脚本已出现拖拽排序标记和 `resume-sortable-item`。
- 兼容边界：未改变简历字段、导入格式、PDF 模板或网申助手字段路径；沿用现有本地/云端保存。
...
- 第二轮视觉复核收紧数据分析指标区：四张独立卡改为一块连续仪表条，保留平板与手机两列断点，减少空白和模板化卡片感。
- 最终确认删除首页四段概览条，线上缓存破除请求已确认相关文案和结构均不存在。
- 发布提交：`142cff3`、`f28b993`、`ee653a9`、`41cecde`、`91120c0` 已推送 `origin/main`；未新增业务数据写入。
...
审计阅读规则：本文中带日期的旧版本、旧路由和旧分享能力只作为历史证据；当前行为以 origin/main、线上页面和 WORKSPACE_STATE_2026-09-14.md 为准。
...
- 真实页面证据：哔哩哔哩 ATS 的学校/公司标签与输入框分离，输入框依赖占位符；旧日期配对逻辑错误覆盖整条记录卡片内的普通文本框。修复后真实 Chrome 登录态 AI 流程完成 16/16 批次，学校、4 段公司名称及职位均可从页面可访问树读回，且无日期落入名称字段；没有保存、上传或提交第三方表单。
- 代码证据：日期配对只接受 date/month/datetime-local、明确日期语义、年/月选择器或只读日期触发器；非 ASCII 属性别名最短长度降为 2；硬事实接受阈值统一为 0.74。新增 controlled-text 与 numbered-ATS 两个夹具，21 个浏览器夹具全部通过；`npm test` 178/178，typecheck、lint（0 error、1 既有 warning）、build（62 routes）、smoke、扩展构建、包校验与 diff check 通过。
- 发布证据：`e75855beebfbb5172d516f7ead7759341e82e899` 已推送 `origin/main`；正式 1.1.3 ZIP 返回 200 / application/zip / 250231 bytes，SHA-256 `5d29da3bbfe2bc820564b7f4fd2cef623c889f0d11acb8e622cd655e02c25c0a`，与仓库包一致。匿名 API 返回 401，只作为鉴权边界证据。
