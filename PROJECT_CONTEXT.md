...
- `ResumeEditor` 为教育、实习/工作、项目三组条目增加拖拽排序，以及上移/下移按钮和删除操作；排序直接写入 `content.education`、`content.work`、`content.projects` 数组。
- 继续使用既有 `touchResume`、自动保存、实时预览和 PDF 导出，不改变导入与网申助手路径；提交 `4725214` 已推送 `origin/main`。
...
- 数据分析指标区已在 `91120c0` 收紧为连续四段仪表条，桌面保持一行，平板与移动端按两列排列，减少独立卡片造成的空白。
- 首页已在 `222bea2` 移除四段概览条，工作模块直接成为首屏主内容。
