...
- 内推码广场首屏改为只读取内推码与公开来源，移除服务端全量 active jobs 查询；岗位选项在用户打开上传抽屉时按需读取。公开来源改用 Next 缓存、并发请求合并、4.5 秒超时和浏览器/边缘 stale-while-revalidate，保留公司筛选、来源追溯和上传权限边界。
...
- 后台反馈列表补充成功状态播报、柔和列表容器和轻量悬停层次，继续支持移动端与 reduced-motion。`npm test -- --runInBand` 183/183、`npm run typecheck`、`npm run lint`、Webpack 构建和 `git diff --check` 均通过；修改尚未提交、推送或部署，Supabase migration 尚未在托管环境执行。
...
当前运行路由与视觉快照优先于本文旧版表格：用户端使用 `/explore`、`/my`、`/bottle`、`/resume`、`/forum`、`/extension`、`/profile`、`/feedback`、`/referrals`；旧表格中出现的星瓶分享海报、旧安装包和旧路由均为历史记录。
...
- 根因不是 AI 没有给出名称，而是执行层对记录卡片内所有文本控件运行日期配对，加上属性匹配器排除了 4 个汉字的学校/公司别名，导致学校、公司、职位被当作起止日期或无法重绑定。
- 1.1.3 将日期推断限制在真实日期候选控件，并支持至少 2 个非 ASCII 字符的明确属性别名；基础硬事实三层阈值统一为 0.74。新增受控文本框和可见记录序号 ATS 夹具。
- 自动测试 178/178、构建/Smoke/包一致性通过；21 个扩展浏览器夹具均通过。真实 Chrome 登录态完成 16/16 AI 批次，页面确认学校、4 段公司和对应职位已写入，未保存或提交。
- 提交 `e75855beebfbb5172d516f7ead7759341e82e899` 已推送；线上 1.1.3 ZIP 为 250231 bytes，SHA-256 `5d29da3bbfe2bc820564b7f4fd2cef623c889f0d11acb8e622cd655e02c25c0a`，与本地一致。
...
- 实际改动：`fill.js` 增加可访问名称、区块路径、邻近文本、控件/交互类型、动态选项等待与匹配、稳定语义指纹、provider 识别和写入后回读；`popup.js` 发送页面区块和岗位上下文，并保持全批次成功后再写入；两条扩展 API 增加字段元数据、页面区块、岗位上下文和 grounded_generation 约束；官网和安装教程同步为 1.0.0，删除旧网申助手图片，新增覆盖模式浏览器夹具。
- 安全边界：规则填写只处理可核对资料；AI 生成仅限基于简历证据的描述类安全字段；拒绝无证据数字、敏感身份信息、教育描述虚构和未知字段；不读取 Cookie，不自动提交，不处理密码、验证码、隐私同意和录用决策题；没有新增数据库迁移或第二套后端。
- 验证：166/166 单元测试、lint 0 errors、typecheck、Webpack 62 路由构建、扩展打包与逐字节校验通过。三种浏览器夹具均通过：只填空白项、覆盖已有内容、AI 智能填写。Chrome headless runner 因当前机器无页面 stdout 未作为通过证据；隔离副本的静态 smoke 通过，临时 dev smoke 受跨目录依赖路径的 Turbopack 限制。
- 本地前端：`http://127.0.0.1:3110/extension` 和 `/extension/guide` 已检查 1.0.0、下载入口、旧图删除及布局，没有发现遮挡或溢出。待推送后再做正式站 HTTP 核验。
