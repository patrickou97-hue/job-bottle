...
5. 获奖名称映射 awards.title，获奖时间映射 awards.date，获奖描述映射 awards.description；证书发证时间映射 certifications.date；项目链接映射 project.url。
6. 出生日期、生日、birth date 或 DOB 分类为 basics.birthDate，年龄分类为 basics.age；性别、国籍/地区、期望工作地点分别分类为 basics.gender、basics.nationality、basics.preferredLocations。是否实际填写由客户端和服务端检查简历中是否有用户明确保存的值。出生地、证件号、验证码、附件、密码必须返回 null。
