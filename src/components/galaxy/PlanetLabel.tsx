...
export function PlanetLabel({ planet }: { planet: PlanetRoute | null }) {
  return (
    ...
    >
      <span className="font-display text-xl font-semibold" style={{ color: 'rgba(255,249,227,0.94)' }}>
        {planet?.label ?? '拾星'}
      </span>
      <span className="max-w-sm text-sm leading-6" style={{ color: 'rgba(216,225,239,0.7)' }}>
        {planet?.description ?? ''}
      </span>
    </div>
  )
