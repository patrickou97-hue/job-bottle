  ...
  if (state === "error" && !data) {
    return (
      <section className="analytics-empty rounded-2xl border border-[#e2e6ed] bg-white px-6 py-16 text-center shadow-[0_12px_32px_rgba(18,41,78,0.05)]">
        <div className="mx-auto flex size-11 items-center justify-center rounded-xl bg-[#eef2f7] text-[#12294e]">
          <Activity aria-hidden="true" className="size-5" />
        </div>
        <h1 className="mt-5 text-lg font-semibold text-[#1d1d1f]">数据分析暂时不可用</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            <select
              ...
              className="h-10 appearance-none rounded-lg border border-[#d7dde6] bg-white px-3 pr-9 text-sm font-medium text-[#25364e] outline-none transition focus:border-[#12294e] focus:ring-2 focus:ring-[#12294e]/10"
            >
  ...
  return (
    <article className="rounded-[14px] border border-[#dde3eb] bg-white px-5 py-5 shadow-[0_8px_24px_rgba(18,41,78,0.04)]">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="flex size-8 items-center justify-center rounded-lg bg-[#eef2f7] text-[#335f98]"><Icon aria-hidden="true" className="size-4" /></span>
      </div>
          ...
          {TREND_OPTIONS.map((option) => (
            <button
              ...
              className={cn(
                ...
                metric === option.value ? "bg-white text-[#12294e] shadow-sm" : "hover:text-[#25364e]",
              )}
            >
              {option.label}
            </button>
            ...
            <svg className="h-full w-full overflow-visible" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${metricLabel}趋势图`} preserveAspectRatio="none">
              <defs>
                <linearGradient id="admin-analytics-area" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="#3567a8" stopOpacity="0.2" />
                  <stop offset="100%" stopColor="#3567a8" stopOpacity="0.02" />
                </linearGradient>
              </defs>
              {[0, 0.5, 1].map((ratioValue) => <line key={ratioValue} x1={padding.left} x2={padding.left + innerWidth} y1={getY(max * ratioValue)} y2={getY(max * ratioValue)} stroke="#edf0f4" strokeWidth="1" />)}
    ...
    <section className="rounded-[14px] border border-[#dde3eb] bg-white px-5 py-5 shadow-[0_8px_24px_rgba(18,41,78,0.04)] sm:px-6">
      <div className="mb-5 flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2.5">
          <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-[#eef2f7] text-[#335f98]"><Icon aria-hidden="true" className="size-4" /></span>
...
function SegmentRow({ segment, index }: { segment: AdminAnalyticsRank; index: number }) {
  const colors = ["#3567a8", "#5f84b4", "#8da6c3", "#d5dce6"];
  return (
