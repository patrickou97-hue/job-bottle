...
begin
  ...
  if operation_exists then
    -- One operation remains capped at 100 batches. The rolling window allows
    -- up to five such operations (500 batches) in ten minutes.
    if current_batch_count >= 100
      or active_batch_count >= 500
  ...
  if active_operation_count >= 5 or active_batch_count >= 500 then
    return false;
  end if;
