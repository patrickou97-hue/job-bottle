...
const SOURCE_INVARIANTS = [
  ...
  },
  {
    ...
    mustInclude: ["server-only", "x-starinterview-client", "x-starinterview-install-id", "x-forwarded-for", "getStarInterviewLLMConfiguration", "DEEPSEEK_API_KEY", "DEEPSEEK_BASE_URL", "DEEPSEEK_MODEL", "https://api.deepseek.com", "deepseek-v4-flash", "getStarInterviewASRConfiguration", "MIMO_API_KEY", "MIMO_ASR_MODEL", "MIMO_ASR_BASE_URL", "https://token-plan-cn.xiaomimimo.com/v1"],
    mustNotInclude: ["NEXT_PUBLIC_", "SUPABASE_SERVICE_ROLE_KEY"],
    label: "诘星 DeepSeek 回答与 MiMo ASR 配置彼此独立且原始模型密钥只存在于服务端",
  },
  {
  ...
  },
  {
    ...
    mustInclude: ["reset", "重试", "返回首页", "当前页面的数据仍在"],
    mustNotInclude: [],
    label: "页面级错误边界提供恢复路径且不误报数据删除",
  },
  {
    file: "src/app/global-error.tsx",
    mustInclude: ["<html", "<body", "reset", "账户数据不会受到影响"],
    mustNotInclude: [],
  ...
  },
  {
    ...
    mustInclude: ["AbortController", "55_000", "isResumePolishResult", "生成结果格式异常", "润色请求超时"],
  ...
  },
  {
    ...
    mustInclude: ["try {", "catch {", "finally {", "岗位星系暂时无法读取"],
    mustNotInclude: [],
  ...
  },
  {
    ...
    mustInclude: ["label: '岗位坐标'", "label: '投递管理'", "label: '简历制作'", "label: '网申助手'", "label: '拾星指南'", "label: '星瓶'"],
  ...
  {
    file: "src/components/resume/ResumeEditor.tsx",
    mustInclude: ["PhotoField", "cropPhotoToPortrait", "上传照片", "复制通用简历后", "智能润色", "ResumePolishDialog", "bg-[#1D2F4F]", "rounded-lg", "focus-visible:ring-[#1D2F4F]/40", "mergeBullets", "撤销", "开始时间（可选）", "结束时间（可选）", "<textarea", "rows={4}", "min-h-28", "resize-y"],
  ...
  },
  {
    ...
    mustInclude: ["role=\"progressbar\"", "aria-valuetext", "elapsedSeconds", "prefersReducedMotion", "onCancel", "处理完成前不会改动原内容"],
    mustNotInclude: ["linear-gradient", "MIMO", "DeepSeek"],
    label: "所有长时 AI 任务共用真实区块进度、已用时、取消操作和原文保护提示",
  },
  {
  ...
  {
    file: "src/components/resume/ResumeBuilderClient.tsx",
    mustInclude: ["ResumeCreateDialog", "ResumeImportDialog", "createResumeFromImport", "createResumeFromTranslation", "requestResumeTranslation", "resume_import_created", "resume_translation_created", "review_mode", "本地识别结果", "复核结果", "导入简历", "新建简历", "转为英文", "转为中文", "原简历未改动", "showImportDialog", "showCreateDialog", "deletedResumeIdsRef", "await cloudSaveWorkerRef.current", "再次点击“删除”以确认", "取消翻译", "translationProgress", "AiTaskProgress", "completed={translationProgress.completed}", "onCancel", "原简历不会改动"],
  ...
  },
  {
    ...
    mustInclude: ["中文简历", "English Resume", "先选择中文或英文", "getDefaultResumeTemplate", "ResumeTemplatePicker", "使用此模板"],
  ...
  },
  {
    ...
    mustInclude: ["china-provinces.json", "geoMercator", "matchesLocationFilter", "province:", "全国省级岗位分布图", "SMALL_REGION_LABELS", "北京:", "天津:", "上海:", "香港:", "澳门:", "group-focus:underline", "aria-pressed", "Enter", "这里暂时没有匹配岗位", "selectedJobs.slice(0, 8)", "查看全部"],
  ...
  },
  {
    ...
    mustInclude: ["generateMetadata", "fetchJobById", "JobDetailActions", "返回岗位坐标", "RelatedJobs", "开放时间", "工作职责", "任职要求", "优先条件", "关键词", "原始岗位信息尚未"],
    mustNotInclude: ["SUPABASE_SERVICE_ROLE_KEY"],
  ...
  },
  {
    ...
    mustInclude: [
      ...
      "appliedPosition",
      "savedAppliedPosition",
      "appliedPositionSaveTimerRef",
      "saveAppliedPosition",
      "handleAppliedPositionBlur",
      "我实际投递的岗位",
      "输入后自动保存",
  ...
  },
  {
    ...
    mustInclude: ["当前经历的全部描述", "仅处理当前段落", "原文", "建议稿", "调整说明", "建议补充", "风险提示", "应用修改", "AI 补充的细节，采用前请核实", "我已逐项核实", "核实后应用", "verificationConfirmed", "保留原文", "重新生成", "CommunityHelpLink", "自定义修改要求（可选）", "customInstruction.trim()", "maxLength={600}", "不填写则只按预设方向润色"],
    mustNotInclude: ["MiMo", "第三方", "MIMO_API_KEY", "MIMO_BASE_URL", "MIMO_MODEL"],
    label: "AI 结果先对比确认且客户端不暴露供应商或服务端配置",
  },
  {
    file: "supabase/migrations/20260830120000_application_position.sql",
    mustInclude: ["applied_position text", "user_applications_applied_position_length_check", "char_length(applied_position) <= 160"],
    mustNotInclude: ["drop table", "delete from public.user_applications"],
    label: "投递记录增加可选的实际投递岗位字段并限制长度",
  },
  {
  ...
  {
    file: "src/components/jobs/JobCard.tsx",
    mustInclude: ["onOpenReferral", "KeyRound", "内推码"],
    mustNotInclude: ["window.prompt", "window.alert"],
    label: "每条岗位清单记录提供独立内推码入口",
  },
  {
    file: "src/components/referrals/ReferralCodeHub.tsx",
    mustInclude: ["内推码广场", "内推码由用户自行分享", "发布后会立即进行一次智能审核", "上传者匿名", "ReferralCodeDrawer", "ReferralUploadDialog", "ReferralReportDialog", "CompanySearchCombobox", "aria-autocomplete", "输入公司名称搜索", "公司匹配结果", "surface-read-bg-strong", "正在上传并审核", "getCreatedMessage", "本地演示模式"],
    mustNotInclude: ["surface-strong-bg", "SUPABASE_SERVICE_ROLE_KEY", "display_name", "phone", "contact_email", "MiMo"],
    label: "内推码广场先公开后立即智能审核，并保留匿名查看、风险提示与举报闭环",
  },
  {
    file: "src/lib/referral-codes.ts",
    mustInclude: ["PROHIBITED_REFERRAL_CONTENT", "validateReferralCodeInput", "LOCAL_REFERRAL_STORAGE_KEY", "REFERRAL_READ_TIMEOUT_MS", "canUseLocalPreview", "referral_code_reports", "fetch(\"/api/referrals\"", "reviewStatus"],
    mustNotInclude: ["service_role", "NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY"],
    label: "内推码客户端拒绝交易和敏感内容并经服务端上传审核，且仅在 localhost 使用演示回退",
  },
  {
    file: "src/app/api/referrals/route.ts",
    mustInclude: ["supabase.auth.getUser()", "createAdminClient", "validateReferralCodeInput", "create_referral_code_for_review", "referral_upload_rate_limit", "claim_referral_code_for_review", "reviewReferralCodeWithMimo", "complete_referral_code_review", "reviewStatus", "Cache-Control"],
    mustNotInclude: ["console.log", "MIMO_API_KEY"],
    label: "内推码上传先公开，再原子登记唯一一次 MiMo 审核并自动下架明确违规内容",
  },
  {
    file: "src/lib/referral-moderation.ts",
    mustInclude: ["MIMO_API_KEY", "MIMO_BASE_URL", "MIMO_MODEL", "enable_thinking: false", "response_format", "job_agency", "career_coaching", "proxy_application", "lead_generation", "confidence >= 0.8", "不可信数据", "证据不足时 keep"],
    mustNotInclude: ["NEXT_PUBLIC_MIMO", "console.log"],
    label: "MiMo 内推码审核采用保守分类、提示注入防护与高置信度下架阈值",
  },
  {
    file: "supabase/migrations/20260817180000_referral_code_plaza.sql",
    mustInclude: ["create table if not exists public.referral_codes", "review_attempts between 0 and 1", "review_due_at timestamptz not null default now()", "referral_codes_select_active", "grant select (id, company_name", "create_referral_code_for_review", "pg_advisory_xact_lock", "interval '10 minutes'", ">= 5", "claim_referral_code_for_review", "claim_referral_codes_for_review", "for update skip locked", "complete_referral_code_review", "list_referral_codes_for_admin", "deactivate_referral_code_as_admin", "public.is_admin()", "referral_code_reports_insert_own", "referral_codes_safe_text_check", "to service_role"],
    mustNotInclude: ["grant select on table public.referral_codes to anon", "referral_codes_insert_own", "grant insert (user_id", "drop table", "delete from public.jobs", "delete from public.user_applications"],
    label: "内推码表隐藏上传者身份、锁定单次审核、补偿漏单并由数据库复核管理员下架权限",
  },
  {
    file: "scripts/review_referral_codes.mjs",
    mustInclude: ["claim_referral_codes_for_review", "p_limit: 100", "reviewReferralCodeWithMimo", "complete_referral_code_review", "upstream_error", "JSON.stringify(counters)"],
    mustNotInclude: ["usage_note", "company_name", "console.log(record"],
    label: "三小时漏单任务只领取尚未调用过模型的记录且不记录用户内容",
  },
  {
    file: ".github/workflows/review-referral-codes.yml",
    mustInclude: ["cron: \"23 */3 * * *\"", "node-version: 24", "review_referral_codes.mjs", "MIMO_API_KEY", "SUPABASE_SERVICE_ROLE_KEY", "cancel-in-progress: false"],
    mustNotInclude: ["pull_request_target", "permissions: write-all"],
    label: "内推码漏单补偿每三小时运行并使用仓库加密密钥",
  },
  {
    file: "src/app/api/admin/referrals/route.ts",
    mustInclude: ["requireAdminAccess", "list_referral_codes_for_admin", "deactivate_referral_code_as_admin", "2–240", "Cache-Control"],
    mustNotInclude: ["createAdminClient", "SUPABASE_SERVICE_ROLE_KEY"],
    label: "内推码管理员 API 通过调用者会话和数据库函数二次校验权限",
  },
  {
    file: "src/components/admin/AdminReferralsClient.tsx",
    mustInclude: ["内推码管理", "智能审核通过", "转人工复核", "举报", "人工下架", "/api/admin/referrals"],
    mustNotInclude: ["window.prompt", "window.alert", "service_role"],
    label: "管理后台可检索审核状态、查看举报并填写原因人工下架",
  },
  {
    file: "src/app/api/admin/users/route.ts",
  ...
  },
  {
    ...
    mustInclude: ["全部用户", "24 小时内活跃", "3 日内活跃", "邮箱、姓名、学校、方向或 ID", "最近活跃", "筛选用户", "账户身份", "普通用户", "管理员", "停用账户", "恢复登录", "确认停用账户", "设为已确认", "确认邮箱", "StarInterview 无限访问", "标准访问", "role=\"switch\"", "仅主管理员可调整此权限"],
  ...
  },
  {
    ...
    mustInclude: ["assets/icon48.png", "拾星网申助手", "只在你点击时填写当前页面", "安全模式", "id=\"pageContext\"", "当前网申页", "只填空白项", "覆盖已有内容", "AI 智能填写", "不会填写验证码、密码或敏感声明", "处理进度", "progressMeter", "role=\"progressbar\"", "progressElapsed", "modeHint", "unmatchedDetails", "footer-main-actions", "aria-hidden=\"true\""],
    mustNotInclude: ["<svg", "emoji"],
    label: "扩展弹窗使用拾星品牌与单层任务流结构",
  },
  {
    file: "browser-extension/starjob-resume-assistant/popup.css",
    mustInclude: ["html {", "width: 380px", "body {", "width: 100%", "overflow-x: hidden", "repeat(3, minmax(0, 1fr))", ".mode-group label:has(input:checked)", ".mode-hint[data-tone=\"warning\"]", ".mode-hint[data-tone=\"ai\"]", ".result-details", ".result-panel", ".progress-panel", ".progress-meter", ".progress-meter-fill", "progress-scan", "min-height: 36px", "prefers-reduced-motion"],
  ...
  },
  {
    ...
    mustNotInclude: ["public/downloads", "拾星网申助手-v0.2.0.zip", "SUPABASE_SERVICE_ROLE_KEY"],
